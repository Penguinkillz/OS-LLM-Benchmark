...
function w(rel, c) {
  const p = path.join("C:\\os-llm-benchmark", "apps", "web", rel);
  fs.mkdirSync(path.dirname(p), { recursive: true });
  fs.writeFileSync(p, c, "utf8");
}

w("db/schema.ts", `import { relations } from "drizzle-orm";
...
w("db/index.ts", `import { drizzle } from "drizzle-orm/node-postgres";
import { Pool } from "pg";
...
function pool() {
  ...
  if (!g.__osllm_pool) g.__osllm_pool = new Pool({ connectionString: url, max: 10 });
